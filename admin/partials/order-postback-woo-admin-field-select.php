...
if ( ! empty( $atts['label'] ) ) {
	...
	?><label for="<?php echo esc_attr( $atts['id'] ); ?>"><?php esc_html_e( $atts['label'], 'pixels' ); ?>: </label><?php

}

?><select
	aria-label="<?php esc_attr( _e( $atts['aria'], 'pixelforms' ) ); ?>"
...
if ( ! empty( $atts['blank'] ) ) {
	...
	?><option value><?php esc_html_e( $atts['blank'], 'pixelforms' ); ?></option><?php
